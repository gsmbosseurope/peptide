<?php
/**
 * Server-side SEO for the Peptide Guide topic page: reads ?id= and looks
 * up the matching topic from peptide-guide-data.js so <title>, meta
 * description, canonical, and Open Graph/Twitter tags reflect the real
 * topic in the raw HTML — not just the client-side document.title set
 * later by peptide-guide.js. This is what search engines and link
...
$pageDesc = $topic && !empty($topic['summary'])
    ...
    : 'In-depth topic guides covering peptide categories, mechanisms, and research use.';
$pageUrl = 'https://trusted-peptide.com/peptide-guide-topic' . ($topic ? '?id=' . rawurlencode($topic['id']) : '');
...
<img src="assets/brand/logo-icon.png" alt="Trusted Peptide" class="brand-logo" width="64" height="64" />
      ...
      <div class="header-actions">
        <a href="<?php echo $topic ? '/ar/peptide-guide-topic?id=' . rawurlencode($topic['id']) : '/ar/peptide-guide'; ?>" class="lang-switch" title="التبديل إلى العربية">عربي</a>
    ...
    <nav class="mobile-nav">
      ...
      <a href="gallery">Gallery</a>
      <a href="contact">Contact</a>
      <a href="<?php echo $topic ? '/ar/peptide-guide-topic?id=' . rawurlencode($topic['id']) : '/ar/peptide-guide'; ?>" class="lang-switch">عربي</a>
    </nav>

  <main class="section container" style="padding-top:48px; max-width:800px;" id="peptide-topic-detail-root">
    <?php if ($topic): ?>
    <!-- Server-rendered fallback: replaced by peptide-guide.js on load, but
         gives search engines and no-JS clients real content immediately
         instead of an empty container. -->
    <div class="breadcrumb">
      <a href="/">Home</a> / <a href="peptide-guide">Peptide Guide</a> / <?php echo htmlspecialchars($topic['title']); ?>
    </div>
