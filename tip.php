<?php
/**
 * Server-side SEO for the Tips & Guide article page: reads ?id= and looks
 * up the matching guide from guides-data.js so <title>, meta description,
 * canonical, and Open Graph/Twitter tags reflect the real article in the
 * raw HTML — not just the client-side document.title set later by
 * guides.js. This is what search engines and link previews actually read.
 */
require_once __DIR__ . '/admin-php/config.php';
require_once __DIR__ . '/admin-php/data.php';

$guideId = isset($_GET['id']) ? (string) $_GET['id'] : '';
$guides = load_guides();
$guide = null;
foreach ($guides as $g) {
    if ($g['id'] === $guideId) { $guide = $g; break; }
}

$pageTitle = $guide ? htmlspecialchars($guide['title']) . ' — trusted-peptide.com' : 'Guide — trusted-peptide.com';
$pageDesc = $guide && !empty($guide['summary'])
    ? htmlspecialchars($guide['summary'])
    : 'Storage, handling, and reconstitution guides for research peptides.';
$pageUrl = 'https://trusted-peptide.com/tip' . ($guide ? '?id=' . rawurlencode($guide['id']) : '');
$pageImage = $guide && !empty($guide['images'][0])
    ? 'https://trusted-peptide.com/' . ltrim($guide['images'][0], '/')
...
<img src="assets/brand/logo-icon.png" alt="Trusted Peptide" class="brand-logo" />
  ...
  <main class="section container" style="padding-top:48px; max-width:800px;" id="guide-detail-root">
    <?php if ($guide): ?>
    <!-- Server-rendered fallback: replaced by guides.js on load, but gives
         search engines and no-JS clients real content immediately instead
         of an empty container. -->
    <div class="breadcrumb">
      <a href="/">Home</a> / <a href="blog">Blog</a>
 <a href="tips">Tips &amp; Guide</a> / <?php echo htmlspecialchars($guide['title']); ?>
    </div>
    <h1><?php echo htmlspecialchars($guide['title']); ?></h1>
    <p><?php echo htmlspecialchars($guide['summary']); ?></p>
    <?php foreach (($guide['body'] ?? []) as $paragraph): ?>
